...
def save_status(status_data):
    """Save processing status to file with error handling"""
    try:
        # Ensure the status data is serializable
        if not isinstance(status_data, dict):
            logger.error(f"Invalid status data type: {type(status_data)}")
            return False
...
def load_status():
    """Load processing status from file or session state with error handling"""
    try:
        # First try to get from session state
        session_status = st.session_state.get('processing_status')
        if session_status:
        ...
        if os.path.exists(STATUS_FILE):
            try:
                ...
                try:
                    ...
                    logger.error(f"Error removing corrupted status file: {str(remove_error)}")
                return None
            except Exception as e:
        ...
        logger.error(f"Unexpected error in load_status: {str(e)}")
        return None

def validate_status(status):
    """Validate status data structure"""
    required_fields = ['current', 'total', 'status', 'timestamp']
...
def process_products_background(generator, df, output_file, status_queue):
    """Background processing function with improved error handling"""
    try:
        ...
        for i, row in df.iterrows():
            try:
                ...
                if not validate_status(status):
                    logger.error("Invalid status data generated")
                    continue
                    
                status_queue.put(status)
                if not save_status(status):
                    logger.error("Failed to save status")
                    continue
                ...
                logger.error(f"Error processing product {row['sku']}: {str(e)}")
                status = {
                    'current': i + 1,
                    'total': total_products,
                    'current_sku': str(row['sku']),
...
def main():
    try:
        ...
        if current_status and validate_status(current_status):
            ...
            if os.path.exists(STATUS_FILE):
                try:
                    ...
                    logger.error(f"Error removing invalid status file: {str(e)}")
